...
class GooglePhotosAPI(GoogleAPI):
    def __init__(self, mongo_uri, credentials_path='credentials.json', token_path='token.json'):
        super().__init__('photoslibrary', 'v1', credentials_path, token_path, SCOPES)
        self.service = self.get_service()
        self.mongo_client = MongoClient(mongo_uri)
        self.mongo_db = self.mongo_client['photos_db']
        self.mongo_collection = self.mongo_db['photos']
    ...
    def download_photo(self, photo, album_name):
        ...
        photo_id = photo['id']
        photo_version = photo['mediaMetadata']['creationTime']
        ...
        result = self.mongo_collection.find_one({"id": photo_id})
        if result and result['version'] == photo_version:
        ...
        # Update the MongoDB database with the new photo
        self.mongo_collection.update_one(
            {"id": photo_id},
            {"$set": {"filename": file_name, "version": photo_version}},
            upsert=True
        )
